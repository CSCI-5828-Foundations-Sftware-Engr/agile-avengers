...
def validate_transaction(transaction_detail, db_session):
    ...
    try:
        ...
        if transaction_method == "bank":
            method = db_session.query(BankAccount).filter_by(account_number=transaction_method_id).first()
            if not method: 
                err_resp = {"message": "Transaction method is incorrect. No corresponding bank account found."}
                return False, err_resp
            if method.account_balance - transaction_amount <= 0:
                err_resp = {"message": "Insufficient funds, transaction cannot take place"}
        elif transaction_method == "credit":
            method = db_session.query(CreditCard).filter_by(card_number=transaction_method_id).first()
            if not method: 
                err_resp = {"message": "Transaction method is incorrect. No corresponding credit card account found."}
                return False, err_resp
            if method.credit_limit - transaction_amount <= 0:
                err_resp = {"message": "Insufficient funds, transaction cannot take place"}
        elif transaction_method == "debit":
            method = db_session.query(DebitCard).filter_by(card_number=transaction_method_id).first()
            if not method: 
                err_resp = {"message": "Transaction method is incorrect. No corresponding debit card account found."}
                return False, err_resp
            bank_detail = db_session.query(BankAccount).filter_by(account_number=method.bank_account_number).first()
            if bank_detail.account_balance - transaction_amount <= 0:
                err_resp = {"message": "Insufficient funds, transaction cannot take place"}
        else:
            err_resp = {"message": "Available transaction methods are bank, credit and debit card."}

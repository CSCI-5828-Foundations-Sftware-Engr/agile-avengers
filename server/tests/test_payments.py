...
from datamodel.models.userinfo import BankAccount, Base, CreditCard, DebitCard, UserInfo
...
class TestPayment:
    ...
    def cleanup_entires(self):
        ...
        self.session.query(CreditCard).filter(
            CreditCard.card_number == "1234567891234568"
        ).delete()
        # delete debit card
        ...
        self.session.query(BankAccount).filter(
            BankAccount.account_number.in_(["1234562315423567", "123456789123423567"])
        ).delete()
        # delete payer and payee info
        self.session.query(UserInfo).filter(
            UserInfo.user_id.in_([self.payer_id, self.payee_id])
        ).delete()
    ...
    def assert_balance(self, res, method):
        ...
        user = (
            ...
            .filter(BankAccount.account_number == "1234562315423567")
            .first()
        )
        ...
        if method == "credit":
            credit = (
                ...
                .filter(CreditCard.card_number == "1234567891234568")
                .first()
            )
    ...
    def test_transaction_completion(self):
        ...
        transaction = (
            self.session.query(Transaction)
            .filter(Transaction.transaction_id == self.transaction_id)
            .first()
        )
